...
function checkEmail(){
	...
	$.ajax({
		...
		success:function(cnt){ //컨트롤러에서 넘어온 cnt값을 받는다
			...
			if(cnt!=1 &&pemail.length>0){ //사용가능한 이메일
				$('.email_ok').css("display","inline-block");
				$('.email_taken').css("display","none");
			
			} else if(cnt==1 && pemail.length>0){ //증복된 이메일
				$('.email_taken').css("display","inline-block");
				$('.email_ok').css("display","none");
				
			} else { //email에 아무것도 입력하지 않을 경우 중복검사 문구 모두 안보이게 설정
				$('.email_ok').css("display","none");
				$('.email_taken').css("display","none");
...
<body>
    ...
    <div class="myform">
        <form action="patientjoin" method="post" id='form'>
            <div class="join">
                이메일<br>
                ...
                <span class= "email_taken" style="color:red; display:none;">중복된 이메일입니다.</span>
                비밀번호<br>
                ...
                <input type="text" id="address" name="roadAddress" placeholder="주소" >
                <div class="input">
